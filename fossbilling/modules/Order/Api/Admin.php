...
class Admin extends \Api_Abstract
{
  ...
  private function sendToRabbitMQ($data)
  {
    ...
    $channel->basic_publish($msg, '', 'orders_fossbilling_to_wordpress_queue');
    ...
    {
      if (isset($data['product_custom_1'])) {
        $required['product_custom_1'] = 'Product Custom_1 (id) not passed';
      } else {
        $required = ['product_id' => 'Product id not passed'];
      }

      if (isset($data['client_custom_1'])) {
        $required['client_custom_1'] = 'Client Custom_1 (id) not passed';
      } else {
        $required['client_id'] = 'Client id not passed';
      }

      $this->di['validator']->checkRequiredParamsForArray($required, $data);

      if (isset($data['client_custom_1'])) {
        $client = $this->di['db']->findOne('Client', 'custom_1 = ?', [$data['client_custom_1']]);
      } else {
        $client = $this->di['db']->getExistingModelById('Client', $data['client_id'], 'Client not found');
      }

      if (isset($data['product_custom_1'])) {
        $product = $this->di['db']->findOne('Product', 'custom_1 = ?', [$data['product_custom_1']]);
      } else {
